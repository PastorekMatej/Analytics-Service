            ...
            Ajouter également une évaluation numérique pour chaque texte sur une échelle de 0 à 100 où :
            ...
            Identifie et quantifie les améliorations significatives identifié à travers les texte
            ...
            ÉVOLUTION_GLOBALE:
                ...
                évaluations numériques par texte:
                    - 2024-09-15: 18/100 (A2)
                    - 2024-09-22: 22/100 (A2)
                    - 2024-09-29: 28/100 (A2+)
                    - 2024-10-06: 35/100 (B1)
                    - 2024-10-13: 41/100 (B1) 
                    - 2024-10-20: 45/100 (B1+)
  ...
  - G07 (articles/déterminants) : ≈8 occurrences dans textes_1–4 → 0 dès texte_5.
  - G06 (prépositions/structures à l’infinitif) : 5 occurrences dans textes_1–4 → 0 dès texte_5.
  - G05 (accords/conjugaisons de base) : 6 occurrences concentrées dans textes_1–2 → 0 dès texte_3.
  - G08 (ordre des mots, placement des pronoms/adverbes) : 3 occurrences dans textes_1–2 → 0 ensuite.
  - G01 (auxiliaire au passé composé) : 2 occurrences au texte_2 → 0 dans tous les textes suivants.
  - V01
  - V03
  - V05
  - S01
  ...
  - S03
            ...
            - type: G07
                ...
                occurrences: 8
                exemples:
                    - "c'est très jolie langue" → "c'est une très jolie langue" (texte_1)
                    - "Je étudie français" → "J'étudie le français" (texte_1)
                    - "voir le Tour Eiffel" → "voir la tour Eiffel" (texte_1)
                    - "je faire beaucoup erreurs" → "je fais beaucoup d'erreurs" (texte_1)
                    - "dans un banque" → "dans une banque" (texte_2)
                    - "des nouveaux amis de autres pays" → "de nouveaux amis d'autres pays" (texte_3)
                    - "je n'ai jamais fait du ski" → "je n'ai jamais fait de ski" (texte_3)
                    - "avoir des bons résultats" → "avoir de bons résultats" (texte_4)

                - type: G06
                pattern: Utilisation incorrecte des prépositions / construction à l’infinitif
                occurrences: 5
                exemples:
                    - "je habite dans Prague" → "j'habite à Prague" (texte_1)
                    - "je continue étudier" → "je continue à étudier" (texte_1)
                    - "C'est important pour moi passer..." → "C'est important pour moi de passer..." (texte_2)
                    - "à les montagnes" → "à la montagne / en montagne" (texte_3)
                    - "retourner aux montagnes le prochain hiver" → "retourner à la montagne l'hiver prochain" (texte_3)

                - type: G05
                pattern: Conjugaison/accords de base (verbe-sujet, genre, adjectifs/déterminants)
                occurrences: 6
                exemples:
                    - "Je regarder les films français" → "Je regarde les films français" (texte_1)
                    - "je comprendre pas tout" → "je ne comprends pas tout" (texte_1)
                    - "je faire beaucoup erreurs" → "je fais beaucoup d'erreurs" (texte_1)
                    - "Mon famille" → "Ma famille" (texte_1)
                    - "c'est bon idée" → "c'est une bonne idée" (texte_1)
                    - "Mon petite frère" → "Mon petit frère" (texte_2)

                - type: G08
                pattern: Ordre incorrect des mots (pronoms/adverbes)
                occurrences: 3
                exemples:
                    - "elle aide moi" → "elle m'aide" (texte_1)
                    - "parler français bien" → "bien parler français" (texte_1)
                    - "Nous avons parlé beaucoup" → "Nous avons beaucoup parlé" (texte_2)

                - type: G01
                pattern: Auxiliaire erroné au passé composé (être/avoir)
                occurrences: 2
                exemples:
                    - "nous avons allé au cinéma" → "nous sommes allés au cinéma" (texte_2)
                    - "nous sommes mangé" → "nous avons mangé" (texte_2)
