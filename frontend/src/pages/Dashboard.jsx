          ...
          <div className="analysis-static-card theme-teal">
            ...
            <div className="analysis-static-content">
              {(() => {
                ...
                if (levelChangeLineIndex !== -1) {
                  const levelLine = lines[levelChangeLineIndex];
                  const levels = levelLine.replace('Level Change:', '').trim();
                  // Try to split by arrow
                  const parts = levels.split(/→|->/);
                  
                  const otherLines = lines.filter((_, i) => i !== levelChangeLineIndex).join('\n');
                  
                  return (
                    <>
                      {parts.length === 2 && (
                        <div className="level-evolution-badge-container">
                          <div className="level-badge from-level">
                            <span className="level-label">FROM</span>
                            <span className="level-value">{parts[0].trim()}</span>
                          </div>
                          ...
                          <div className="level-badge to-level">
                            <span className="level-label">TO</span>
                            <span className="level-value">{parts[1].trim()}</span>
                          </div>
                        </div>
                      )}
                      {parts.length !== 2 && (
                        <div className="level-change-simple">
                          <strong>Level Change:</strong> {levels}
                        </div>
                      )}
                      <div className="analysis-text mt-4">
                        {formatSectionContent(otherLines, 'teal')}
                      </div>
                    </>
                  );
                }
